const fs = require('fs');
let content = fs.readFileSync('src/components/gst/ReportsView.tsx', 'utf8');
...
content = content.replace(
  /<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">[\s\S]*?<\/div>[\s\S]*?<div className="space-y-4">[\s\S]*?<\/h3>[\s\S]*?<div className="h-64 rounded-md border p-4 flex items-end justify-between gap-2 bg-white">[\s\S]*?<\/div>[\s\S]*?<\/div>/m,
  advancedUI
);

fs.writeFileSync('src/components/gst/ReportsView.tsx', content);
